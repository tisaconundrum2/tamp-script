...
(function initScripting() {
	...
	var contextSwitch;
	var rawCurrentRecord;
	var currentRecordModule = null;
	var refletModule = null;
	var v2ScriptLoader = null;
	var runRecordScript = function noop() { return true; };
	var includeN = (function ()
	{
		...
		function doIncludeN(debug) {
			...
			try {
				 ...
				 */
				require(['N'], function () {});
				require(['N/contextSwitch'], function(contextSwitchModule) { contextSwitch = contextSwitchModule; });
				require(['N/rawCurrentRecord'], function(rawCurrentRecordModule) { rawCurrentRecord = rawCurrentRecordModule; });
				require(['N/currentRecord'], function(currentRecMod) { currentRecordModule = currentRecMod; });
				require(['N/localeContext'], function(localeContextModule) { localeContext = localeContextModule; });
				require(['N/restricted/reflet'], function(reflet) { refletModule = reflet; });
				require(['N/scriptLoader'], function (scriptLoader) { v2ScriptLoader = scriptLoader; });
				if(hasV2RecordOwnerEventHandler()) {
					require(['N/currentRecord', 'N/rawCurrentRecord', 'N/util/callRecordScript'], function(currentRecord, rawCurrentRecord, callRecMod) {
						var cr = currentRecord.get();    // make sure rawCurrentRecord is initialized
						runRecordScript = callRecMod.create({context: rawCurrentRecord.get().getSystemId(), handlerModules: _eventHandlerModules, scriptingContext: _scriptingContext}).bind(null, cr);
					});
				}
			}
			finally {
			...
			if(hasV2RecordOwnerEventHandler() && runRecordScript == null)
			{
				throw nlapiCreateError(UNEXPECTED_ERROR, 'Record Script 2.0 is not initialized properly.');
			}
	...
	function stillLoadingScripts() {
		return moduleKeeper && moduleKeeper.isLoadingModules();
	}
	...
	callClientScript = (function () {
		...
		function doCallClientScript(trigger, args, options) {
			options = options || {};
			var callback = isFunction(options.callback) ? options.callback : function noop() {};
			var returnResult = true;

			if (isNewUI()
				|| ((trigger === triggerInfo.localizationContextEnter.name || trigger === triggerInfo.localizationContextExit.name)
					&& (!moduleKeeper || !_localeContext || !isRecordInitialized() || options.locale !== _localeContext.getTargetLocale()))
				)
			{
				callback({executed:false, trigger: trigger, locale: options.locale});
				return returnResult;
			}
			...
			if (hasV2clientScripts()) {
				...
				if ((trigger === triggerInfo.localizationContextEnter.name || trigger === triggerInfo.localizationContextExit.name) && stillLoadingScripts())
				{
					if (typeof setTimeout !== "undefined")
						setTimeout(callClientScript, 0, trigger, args, options);
					else
						throw new Error("Failed to load record customizations");
					return;
				}
				returnResult = runClientScript(trigger, args, options.locale);
			}
			else {
				returnResult = oldScriptCall(trigger, args);
			}
			callback({executed:true, trigger: trigger, locale: options.locale});
			return returnResult;
		}
	...
	callRecordScript = (function () {
		function doCallRecordScript(trigger) {
			return runRecordScript(trigger);
		}
		return doCallRecordScript;
	})();
	...
	fetchClientScripts = (function ()
	{
		...
		function doFetching(debug) {
			...
			if (!ranAlready && includedVersion2plusScript()) {
				...
				_localeContext = localeContext.createLocaleContext({
	...
	runClientScript = (function () {
		...
		function doRunClientScript(trigger, _args, locale)
		{
			var args = _args || [];
			var isValid = true;
			...
			try
			{
				...
				for (var i = 0; i < moduleInfos.length; i++) {
					...
					var realTrigger = ((thisVersion === "2.0") && (trigger === triggerInfo.recalc.name)) ? "sublistChanged" : trigger;
					...
					if (shouldRun && !!functionTrigger && !!validCall)
					{
						...
						if (typeof(currentClientScriptFunction) === "function")
						{
							...
							}.bind(this, currentClientScriptFunction, currentArguments));
	...
	function localizationContextEnter(locale)
	{
		return new Promise(function (resolve, reject) {
			var args = [locale];
			if(hasV2RecordOwnerEventHandler()) {
				callRecordScript(getV2RecordScriptOptions(triggerInfo.localizationContextEnter.name, null, null, args));
			}
			callClientScript(triggerInfo.localizationContextEnter.name, args, {locale: locale, callback: resolve});
		});
	}
	...
	function localizationContextExit(locale)
	{
		return new Promise(function (resolve, reject) {
			var args = [locale];
			if(hasV2RecordOwnerEventHandler()) {
				callRecordScript(getV2RecordScriptOptions(triggerInfo.localizationContextExit.name, null, null, args));
			}
			callClientScript(triggerInfo.localizationContextExit.name, args, {locale: locale, callback: resolve});
		});
	}
...
function nlapiFieldChanged(type, fldnam, linenum, column, optionswin)
{
	...
	_localeContext && _localeContext.checkLocaleAndTriggerContextChange({sublistId:type, fieldId:fldnam, valueGetter: nlapiGetFieldValue});
}
...
function nlapiValidateDelete(type)
{
    try
    {
        ...
        isValid = isValid && handleValidationResult(callClientScript(triggerInfo.validateDelete.name, [type]));
        return isValid;
    }
    finally
    {
...
function nlapiEncrypt(s, type, key)
{
    ...
    if (type == null)
        type = "sha1";
    return nsServerCall(nsJSONProxyURL, "encrypt", [s, type, key != null ? key : null], null, 'POST');
}
function nlapiDecrypt(s, type, key)
{
    nsapiCheckArgs( [s], ['encryptedtext'], 'nlapiDecrypt' );
    if ( type == "aes" || type == "base64" || type == "xor" )
        return nsServerCall(nsJSONProxyURL, "decrypt", [s, type, key != null ? key : null], null, 'POST');
    return null;
}
...
function nsapiCallUserScript(trigger, scriptid, func, args)
{
    ...
    try
    {
	    ...
	    if (includedVersion2plusScript())
	    	require('N/restricted/reflet').setupScript({scriptId: scriptid});
...
function nsapiCallScript(trigger, scriptid, func, args)
{
	...
	if (includedVersion2plusScript())
	{
		...
		var rawCurrentRecord = require('N/rawCurrentRecord');

		// make sure rawCurrentRecord is initialized
		currentRecord.get();
		...
		execute = contextSwitch.execute.bind(undefined, context);
	}
	else
	{
